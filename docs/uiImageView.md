...
- Only property is ContentMode (Center / Fit[default] / Fill).
...
- `uiImageViewSetContentMode(iv, mode)` — `Center | Fit(default) | Fill`
...
- **Fill**: Aspect-fill scaling (crop). Image is scaled to fill the entire view while maintaining aspect ratio. Parts of the image may be cropped if aspect ratios don't match.
...
- Interpolation uses high-quality filtering on all platforms:
  - macOS: Uses Core Animation's high-quality scaling for Fill mode, NSImageView's built-in scaling for Center/Fit modes
