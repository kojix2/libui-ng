...
```c
void uiDrawImage(uiDrawContext *c, const uiImage *img,
...
Table image cells use the same shared matcher, with their logical image size
and display DPI as the target.
...
- Image surfaces use bilinear filtering, destination clipping, and padded
  edge sampling so opaque edge pixels remain opaque when scaled.
