...
void uiToolbarSetDisplayMode(uiToolbar *t, uiToolbarDisplayMode mode)
{
	...
	if (!validDisplayMode(mode)) {
		uiprivUserBug("Invalid uiToolbar display mode %d.", (int) mode);
		return;
	}
	t->displayMode = mode;
}
...
{
	uiWindow *window = item->toolbar->window;

	if (!uiprivUserCallbackEnter(window == NULL ? NULL : uiControl(window)))
		return;
	if (item->type == uiprivToolbarItemToggleButton)
		item->checked = checked != 0;
