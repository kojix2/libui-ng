...
#ifdef _WIN32
static void toolbarButtonHasIcon(void **state)
{
	struct state *s = *state;
	...
	uiToolbarAppendButton(toolbar, "Icon", NULL);
	uiWindowSetToolbar(s->w, toolbar);
	...
	assert_true(SendMessageW(nativeToolbar, TB_GETITEMRECT, 0,
		(LPARAM) &iconRect));
	assert_true(SendMessageW(nativeToolbar, TB_GETITEMRECT, 1,
		(LPARAM) &textRect));
	assert_true(iconRect.right - iconRect.left > textRect.right - textRect.left);
...
#ifdef _WIN32
		toolbarUnitTest(toolbarButtonHasIcon),
#endif
