...
static LRESULT CALLBACK uiImageViewWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT: {
		...
		if (v->rt) {
			...
			if (v->bmp) {
				...
				v->rt->DrawBitmap(v->bmp, &dst, 1.0f, D2D1_BITMAP_INTERPOLATION_MODE_CUBIC, nullptr);
			}
	...
	case WM_SIZE:
		...
		InvalidateRect(v->hwnd, NULL, TRUE);  // Size change requires repaint
		return 0;
	case WM_ERASEBKGND:
...
uiImageView *uiNewImageView(void)
{
	...
	// Set USERDATA since WM_CREATE was already processed before WndProc replacement
	...
	InvalidateRect(v->hwnd, NULL, TRUE);  // Initial repaint after setup
...
void uiImageViewSetImage(uiImageView *v, const uiImage *image)
{
	...
	if (v->rt != NULL) {
		...
		if (v->bmp == NULL) {
			// If conversion failed, try to recreate render target
			v->rt->Release();
			v->rt = makeHWNDRenderTarget(v->hwnd);
			if (v->rt != NULL) {
				v->bmp = uiprivImageToD2DBitmap((uiImage *)image, v->rt);
