...
static void buildImages(uiToolbar *t)
{
	...
	// Attach the image list before adding buttons so BTNS_AUTOSIZE calculates
	// each button width from both its final image and text.
...
void uiprivToolbarPlatformAttach(uiToolbar *t, uiWindow *w)
{
	...
	hwnd = CreateWindowExW(0, TOOLBARCLASSNAMEW, NULL,
		...
		TBSTYLE_TOOLTIPS | TBSTYLE_LIST | CCS_NODIVIDER |
	...
	SendMessageW(hwnd, TB_SETEXTENDEDSTYLE, 0, TBSTYLE_EX_MIXEDBUTTONS | TBSTYLE_EX_DOUBLEBUFFER);
	buildImages(t);
	for (size_t i = 0; i < t->len; i++) {
		uiToolbarItem *item = t->items[i];
		TBBUTTON button;
		...
		button.iBitmap = item->nativeImage;
