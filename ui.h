 ...
 *
 * @note Deleting the index of the item currently selected will clear the
 * selection to `-1`.
 *
 * @param c uiCombobox instance.
 ...
 */
_UI_EXTERN void uiDrawImage(uiDrawContext *c, const uiImage *img, double x, double y, double width, double height);
 ...
 * Platform or user preferences may change the effective native presentation.
 */
 ...
 * returned by uiToolbarGetDisplayMode(), even if the native presentation is
...
/** Appends a fixed-width native space. */
_UI_EXTERN void uiToolbarAppendSpace(uiToolbar *t);

/** Appends a space that consumes available horizontal room. */
_UI_EXTERN void uiToolbarAppendFlexibleSpace(uiToolbar *t);
 ...
 *              Invalid values report a user bug and leave the current mode
 *              unchanged.
 * @memberof uiImageView
 */
