...
static void syncCustomButtonContent(uiToolbarItem *item, NSButton *button,
	NSString *text)
{
	[button setTitle:item->icon == NULL ? text : @""];
	[button setImagePosition:item->icon == NULL ? NSNoImage : NSImageOnly];
}
...
static void syncItem(uiToolbarItem *item)
{
	...
	/* A custom text button supplies its own visible label. */
	[native setLabel:customButton && item->icon == NULL ? @"" : text];
